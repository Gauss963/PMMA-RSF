#!/usr/bin/env python3
"""Generate 16 TS0163 normal-dip cases outside the completed 10% sweep."""
...
CASE_COUNT = 16
CASES_PER_DIRECTION = CASE_COUNT // 2
FIRST_RUN = 224
INNER_DEVIATION = 0.10
OUTER_DEVIATION = 0.20
...
def normal_displacement_fractions(index: int) -> tuple[float, float]:
    """Return one new endpoint pair, excluding the completed +/-10% range."""
    if not 1 <= index <= CASE_COUNT:
        raise ValueError(f"Sweep index must be 1 through {CASE_COUNT}, found {index}.")

    step = (OUTER_DEVIATION - INNER_DEVIATION) / CASES_PER_DIRECTION
    if index <= CASES_PER_DIRECTION:
        deviation = OUTER_DEVIATION - (index - 1) * step
        return 1.0 - deviation, 1.0 + deviation

    deviation = INNER_DEVIATION + (index - CASES_PER_DIRECTION) * step
    return 1.0 + deviation, 1.0 - deviation


def case_path(index: int) -> Path:
    run_number = FIRST_RUN + index - 1
    return ROOT / "cases" / f"rsf_{run_number:04d}_normal_dip20_outer_{index:02d}.toml"
...
def render_case(template: str, index: int) -> str:
    ...
    text = replace_once(
        ...
        f'name = "pmma-rsf-{run_number:04d}-normal-dip20-outer-{index:02d}of16"',
    )
    text = replace_once(
        text,
        ...
        (
            "# Outer normal-dip sweep uses the full rectangular moving block.\n"
    ...
    text = replace_once(
        ...
        (
            "normal_displacement = 0.691620986687549\n"
            f"# Outer normal-dip sweep {index:02d}/16; y=0 is the loading end.\n"
