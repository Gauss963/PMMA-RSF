...
run_number=$((271 + SWEEP_INDEX))
printf -v run_id "TS%04d" "$run_number"
printf -v case_name "rsf_%04d_czm_xc_%02d.toml" \
...
echo "$run_id CZM-Xc sweep task started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "$run_id CZM-Xc sweep task ended at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
