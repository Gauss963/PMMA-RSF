...
run_number=$((143 + SWEEP_INDEX))
printf -v run_id "TS%04d" "$run_number"
printf -v case_name "rsf_%04d_shear_rate_%02d.toml" \
...
echo "$run_id shear-rate sweep task started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "$run_id shear-rate sweep task ended at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
