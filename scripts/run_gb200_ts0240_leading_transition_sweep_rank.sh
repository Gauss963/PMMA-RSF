...
run_number=$((255 + SWEEP_INDEX))
printf -v run_id "TS%04d" "$run_number"
printf -v case_name "rsf_%04d_leading_transition_%02d.toml" \
...
echo "$run_id leading-transition sweep task started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "$run_id leading-transition sweep task ended at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
