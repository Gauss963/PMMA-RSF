...
run_number=$((159 + SWEEP_INDEX))
printf -v run_id "TS%04d" "$run_number"
printf -v case_name "rsf_%04d_leading_interp_%02d.toml" \
...
echo "$run_id leading-edge sweep task started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "$run_id leading-edge sweep task ended at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
