...
run_number=$((223 + SWEEP_INDEX))
printf -v run_id "TS%04d" "$run_number"
printf -v case_name "rsf_%04d_normal_dip20_outer_%02d.toml" \
...
echo "$run_id outer normal-dip sweep task started at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "$run_id outer normal-dip sweep task ended at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
